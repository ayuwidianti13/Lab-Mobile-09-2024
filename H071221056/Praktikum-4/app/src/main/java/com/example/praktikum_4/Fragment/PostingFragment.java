...
public class PostingFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Mendaftarkan ActivityResultLauncher untuk mengakses galeri dan memilih gambar
        ActivityResultLauncher<Intent> launcherIntentGallery = registerForActivityResult(
            new ActivityResultContracts.StartActivityForResult(), result -> {
                if (result.getResultCode() == Activity.RESULT_OK) {
                    Intent data = result.getData();
                    if (data != null) {
                        selectedImageUri = data.getData(); // URI gambar dipilih disimpan dalam variabel selectedImageUri
        ...
        btn_posting.setOnClickListener(btn ->{
            ...
            if (!konten.isEmpty()) {
                if (selectedImageUri != null) {
                    Instagram instagram = new Instagram("sun.flowers", "SUNFLOWERS", konten, R.drawable.profile3, selectedImageUri);

                    Bundle bundle = new Bundle();
                    bundle.putParcelable(HomeFragment.EXTRA_INSTAGRAM, instagram); //mengirim data objek Instagram ke HomeFragment.
